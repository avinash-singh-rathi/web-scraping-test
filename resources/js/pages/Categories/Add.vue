          ...
          <div class="form-row">
            <div class="form-group col-md-6">
              ...
              <div class="" :class="{ 'has-danger': submitted && errors.has('name') }">
                <input type="text" class="form-control" v-model="category.name" id="name" name="name" v-validate="'required'" :class="{ 'is-invalid': submitted && errors.has('name') }" placeholder="Enter Name">
              </div>
            ...
            <div class="form-group col-md-6">
              ...
              <div class="" :class="{ 'has-danger': submitted && errors.has('url') }">
                  <input type="url" class="form-control" v-model="category.url" id="url" name="url" v-validate="'required|url'" :class="{ 'is-invalid': submitted && errors.has('url') }" placeholder="https://google.com">
              </div>
      ...
      this.$validator.validate().then(valid => {
        if (valid) {
          this.CreateCategory(this.category).then(response => {
            ...
            this.$swal.fire(
                'Error!',
                'Unable to create category!'+printData,
                'error'
            );
          });
        }else{
